            ...
            <form wire:submit.prevent="requestPassword" class="mt-8 bg-white rounded-lg shadow-xl overflow-hidden">
                <div class="px-10 py-12">
                    ...
                    @if(session()->has('status'))
                        <div class="mt-5 text-sm text-green-500 font-semibold">{{ session('status') }}</div>
                    @endif

                    <x-text-input wire:model.debounce.500ms="email" :error="$errors->first('email')" id="email" label="E-mel" type="email" group-class="mt-5" autofocus autocapitalize="off" />
                </div>
